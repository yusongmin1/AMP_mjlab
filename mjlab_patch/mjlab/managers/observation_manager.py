...
class ObservationGroupCfg:
  ...
  """History layout ordering when ``concatenate_terms=True``.

  - ``"term"``: Term-major ordering (default). Each term's history is
    flattened first, then concatenated:
    ``[A_t0, A_t1, ..., B_t0, B_t1, ...]``
  - ``"time"``: Time-major ordering. All terms at the same timestep are
    grouped together:
    ``[A_t0, B_t0, ..., A_t1, B_t1, ...]``
  """
...
class ObservationManager(ManagerBase):
    ...
    for group_name, group_term_dims in self._group_obs_term_dim.items():
      if self._group_obs_concatenate[group_name]:
        ...
        if len(term_dims.shape) > 1:
          ...
          # Flatten multi-dim (e.g. (H, D) from unflatten history) into (H*D,)
          term_dims = (int(torch.prod(term_dims)),)
        else:
          term_dims = torch.sum(term_dims, dim=0)
        self._group_obs_dim[group_name] = tuple(term_dims.tolist()) if isinstance(term_dims, torch.Tensor) else term_dims
      else:
  ...
  def compute_group(
    ...
    if self._group_obs_concatenate[group_name]:
      ...
      # Flatten 3D (B, H, D) to 2D (B, H*D) for time-major history layout.
    ...
    for group_name, group_cfg in self.cfg.items():
      ...
      for term_name, term_cfg in group_cfg.terms.items():
        ...
        if group_cfg.history_length is not None:
          ...
          # time-major ordering requires unflatten history so terms can be
          # interleaved by timestep during concatenation.
