"""RL configuration for Unitree Go2 AMP locomotion task."""
...
_MOTION_DATA_DIR = os.path.join(
  ...
  "src", "assets", "motions", "go2", "amp",
)


@dataclass
class RslRlAmpPpoAlgorithmCfg(RslRlPpoAlgorithmCfg):
  """PPO algorithm config (symmetry optional / off by default for Go2)."""
...
class RslRlAmpRunnerCfg(RslRlOnPolicyRunnerCfg):
  ...
  min_normalized_std: List[float] = field(default_factory=lambda: [0.05] * 12)


def go2_amp_ppo_runner_cfg() -> RslRlAmpRunnerCfg:
  """Create RL runner configuration for Unitree Go2 AMP locomotion."""
  return RslRlAmpRunnerCfg(
    ...
    experiment_name="go2_amp_locomotion",
    logger="tensorboard",
    save_interval=100,
