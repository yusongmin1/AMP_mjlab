...
Usage:
  python scripts/play_motion_csv.py                       # browse folder
  python scripts/play_motion_csv.py --file <motion.csv>   # specific file
...
def main() -> None:
  ...
  parser.add_argument("--fps", type=float, default=120.0, help="CSV frame rate")
  parser.add_argument("--speed", type=float, default=1.0, help="Initial speed factor")
  ...
  player = MotionPlayer(files[cur], args.xml, args.fps, args.speed, loop=not args.no_loop)
  ...
  state = {
    ...
    "files": files,
    "cur": cur,
    "speed_i": SPEEDS.index(player.speed),
    "quit": False,
    "dirty": True,
  }
  ...
  def on_key(key: int) -> None:
    ...
    if key == K_SPACE:
      ...
      p.step_frame(+1)
    elif key in (ord("w"), ord("W"), K_UP):
      state["speed_i"] = min(state["speed_i"] + 1, len(SPEEDS) - 1)
      p.speed = SPEEDS[state["speed_i"]]
    elif key in (ord("s"), ord("S"), K_DOWN):
      state["speed_i"] = max(state["speed_i"] - 1, 0)
      p.speed = SPEEDS[state["speed_i"]]
      ...
      state["cur"] = (state["cur"] + 1) % len(state["files"])
      state["player"] = MotionPlayer(
        state["files"][state["cur"]], args.xml, args.fps, p.speed, p.loop,
        model=p.model, data=p.data,
      )
  ...
  with mujoco_viewer.launch_passive(player.model, player.data, key_callback=on_key) as v:
    ...
    print(
      ...
      f"({player.duration:.1f}s)"
    )
    print("Space pause | ←/→ frame | ↑/↓ speed | R restart | L loop | N/P file | Q quit\n")
    ...
    while v.is_running() and not state["quit"]:
      ...
      if p.name != shown_name:
        shown_name = p.name
        print(
          f"\n{p.name}: {p.frames} frames @ {p.fps:.0f}fps ({p.duration:.1f}s)"
        )
      ...
      if not p.paused:
        ...
        if now >= next_frame_t:
          p.step_frame(+1)
          next_frame_t = now + frame_dt / p.speed
